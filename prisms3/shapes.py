...
class Shape3:
    ...
    def get_projected_points(self, points, projection_matrix):
        """
        This function multiplies a projection matrix to a list of points.
            @param list of 3-tuples
            @param linear transformation instance
            @return list of projected 2-tuples
        """
        projected_points = []
        for point3 in points:
            point2 = projection_matrix.apply(tuple(point3))
            projected_points.append(tuple(point2))
        return projected_points

    def get_transformed_points(self, *linear_transformations):
        """
        Applies transformation matrices to points in the shape.
            @param linear transformations
            @return 3-tuples
        """
        transformed_points = []
        for point3 in self.points:
            ...
            for linear_transformation in linear_transformations:
                point = linear_transformation.apply(tuple(point))
            transformed_points.append(tuple(point))
    ...
    def render(self, surface, projection_transformation, *linear_transformations):
        """
        Blit to the target surface.
            @param target surface
            @param projection matrix (x, y, z) -> (x, y)
        """
        transformed_points = self.get_transformed_points(*linear_transformations)
        projected_points = self.get_projected_points(transformed_points, projection_transformation)
        if len(self.color)  == 4:
            # (RGB[A])
